...
from pose_transformations import poses_as_outlines, full_pose_as_regression, pose_centers, \
    pose_centers_subset, size_by_regression, slope_of_two_joints, TimeSeriesPoint, Point, normalized_symmetric_pose, \
    normalized_pose, normalized_looping_pose, looping_pose_interpolated
...
class Plot2D:
    def __init__(
        ...
        self.ax.yaxis.set_ticks([-0.2, 0, 0.2])
        ...
        self.extra_points: dict[int, [Point]] = additional_points
...
if __name__ == '__main__':
    ...
    normalized = normalized_pose(regressed_pose, pose_map, pose_size)
    # poses = poses_as_outlines(normalized_symmetric_pose(normalized, shoulder_slope))
    poses = poses_as_outlines(normalized_looping_pose(normalized, shoulder_slope))
    # poses = poses_as_outlines(looping_pose_interpolated(normalized, shoulder_slope))

    # mean_slope = mean(shoulder_slope)
    # st_dev = stdev(shoulder_slope)
    total_frames = len(poses)

    # title_label = "Scaled/Centered Poses With Steps"
    title_label = "Normalized Walking Cycle"
    ...
    instance = Plot2D(
        ...
        center_on=origins,
        pose_sizes=[pose_size[0]] * total_frames
    )
    pose_animation = FuncAnimation(fig, instance, frames=total_frames, interval=int(1000 / total_frames))
    plt.show()
